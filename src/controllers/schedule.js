...
const ScheduleController = (
  ...
  router.post('/', async (req, res) => {
    ...
    const [user_id, user_err] = await authService.getLoggedInUserID(
      req.headers,
    );
    if (user_id == null) {
      return res.status(400).json({
    ...
    const [tasks, task_err] = await taskModel.getTasksForScheduling(
      user_id,
      req.body.ids,
    );
    if (task_err != null) {
    ...
    const [
      googleBusyInts,
      int_err,
    ] = await googleAPIService.getFreeBusyIntervalsWithToken(
      req.headers,
      freeBusyBody,
    );
    if (int_err != null) {
  ...
  router.post('/confirm', async (req, res) => {
    ...
    for (let i = 0; i < goodIDs.length; i++) {
      ...
      let taskStartStringTZ = taskStartMoment
        .tz(req.body.timeZone)
        .toISOString(true);
      let taskEndStringTZ = taskEndMoment
        .tz(req.body.timeZone)
        .toISOString(true);
      ...
      let [
        response,
        calendarErr,
      ] = await googleAPIService.scheduleEventWithToken(
        req.headers,
        targetCalendar,
        reqBody,
      );
      ...
      let [confirmed_task, confirmErr] = await taskModel.confirmSchedule(
        id,
        response.id,
        targetCalendar,
        response.htmlLink,
      ...
      if (confirmErr) console.log('Error confirming task!', confirmErr);
      else confirmedTaskList.push(confirmed_task);
    }
